...
class Auth{
    ...
    {
        if (!$model) {
            $model = Config::get('Config.user_auth_model');
        }
    ...
    public function __construct() {
        $this->request = Request::instance()->request();
    ...
    public function check($name, $uid, $type=1, $mode='url', $relation='or') {
        ...
        if ($mode=='url') {
            $this->request = unserialize( strtolower(serialize($this->request)) );
        }
        foreach ( $authList as $auth ) {
            $query = preg_replace('/^.+\?/U','',$auth);//获取url参数
            if ($mode=='url' && $query!=$auth ) {
                parse_str($query,$param); //解析规则中的param,//获取数组形式url参数
                $intersect = array_intersect_assoc($this->request,$param);
